...
```powershell
# 이미지 빌드
docker build -f infra/docker/Dockerfile -t shim:1.9.9 -t shim:latest .
...
| **버전 동기화** | `.\tools\scripts\release.ps1 -Version 1.9.9` | 릴리즈 시 필수 실행 |
| **검사 및 Docker 빌드** | `.\tools\scripts\release.ps1 -Version 1.9.9 -RunChecks -BuildImage` | CSS·검사 통과 후 이미지 생성 |
