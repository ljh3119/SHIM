...
본 시스템은 다수의 계약 형태(정규직, 프리랜서, 계약직 등)와 가산 연차 예외 사원들을 유연하고 안전하게 관리하기 위해 **"연도별 연차 격리 방식"**을 채택하고 있습니다.

### 1) 연도별 연차 독립 관리 (Isolating Annual Leave)
- 특정 사원의 올해(예: 2026년) 연차 개수를 개별적으로 변경(포상 휴가 추가 등)하더라도, 내년(예: 2027년)이나 과거 연도 연차에는 **어떠한 영향도 미치지 않습니다.**
- 모든 연차 정보는 연도별 레코드로 데이터베이스에 안전하게 격리되어 관리됩니다.

### 2) 연말/연초 필수 작업: 새해 연차 일괄 지급
- 새해로 연도가 전환되는 시기(매년 12월 말 ~ 1월 초)에는 관리자 대시보드 상단에 **연차 설정 안내 배너**가 자동으로 노출됩니다.
- 관리자는 안내에 따라 **[사원 관리 > 연차 일괄 지급]** 기능을 이용하여 새해 연도(예: 2027)를 지정하고 전사 사원에게 기본 연차 일수(예: 15일)를 일괄 부여해야 합니다.
- 근속이 늘어나 가산 연차가 적용되는 대상자나 프리랜서 등 예외 인원은 일괄 지급 완료 후, **[사원 관리]** 리스트에서 해당 사원의 올해 연차 일수만 개별 수정하여 반영해 주시면 됩니다.
